...
export class Context {
    ...
    public getParameterProperties(typeId: string, namesOnly: boolean = false) : Array<any> {
        ...
        let t_cpy: DefinitionType | ObjectType | UnionType | LiteralType | EnumType | IntersectionType = t;


        while (
            // @ts-ignore
            t_cpy.getType != undefined && t_cpy.getType().getProperties == undefined
        ...
        } else if(t_cpy.constructor.name === "IntersectionType") { // pick, values to pic
            //@ts-ignore
            return _.flatMap(_.map((<IntersectionType>t_cpy).getTypes(), a => a.getType().getProperties()));
